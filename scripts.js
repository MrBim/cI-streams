$(document).ready(() => {
    console.log("Ready");
    const arrayOfChords = ["D#m(maj9)"];

    const pickChord = (chord) => {
        const markUp = `<ins class="scales_chords_api" chord="${chord}" instrument="piano"></ins>`;
        return markUp;
    };

    $("#options").on("change", function (e) {
        const newChord = e.target.value;
        const location = $("#chord-box");

        location.html(pickChord(newChord));
        console.log("newChord: ", newChord);
        scales_chords_api_onload();
    });

    $("#options2").on("change", function (e) {
        const newChord = e.target.value;
        const location = $("#chord-box2");

        location.html(pickChord(newChord));
        console.log("newChord2: ", newChord);
        scales_chords_api_onload();
    });
    ...
    function playAudio3(id_number) {
        ...
        button.style.background =
            "url('https://www.scales-chords.com/images/snd-mute-green-2.png') no-repeat";
    ...
    function audioStop(id_number) {
        var button = document.getElementById("playbut_" + id_number);
        button.style.background =
            "url('https://www.scales-chords.com/images/snd-play-green-2.png') no-repeat";
        button.style.backgroundSize = "100%";
    }
    ...
    function ajaxCall(url, params, successCallback, failCallback, ongoingCallback, cfg) {
        ...
        if (typeof params === "string") paramString = params;
            ...
            for (var p in params) {
                var pValue = params[p];
                if (typeof pValue === "string") pValue = encodeURIComponent(pValue);
        ...
        if (cfg) {
            if (cfg.options) {
                for (var o in cfg.options) {
                    if (o in xhr) xhr[o] = cfg.options[o];
                }
            }
            if (cfg.headers) {
